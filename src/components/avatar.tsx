...
const Avatar = styled.div<Props>`
    border-radius: 50%;
    border: 2px #FFF;
    box-shadow: ${({ theme }) => theme.shadows.shadowMedium};
...
function AvatarComponent({size, color}: Props) {
    return (
        <Avatar color={color} size={size}>
            <AvatarPlaceholder />
            <img draggable={false} src="https://avatars.githubusercontent.com/u/188230" alt="avatar" />
        </Avatar>
    )
}
